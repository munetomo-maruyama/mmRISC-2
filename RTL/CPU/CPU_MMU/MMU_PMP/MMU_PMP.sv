...
module MMU_PMP
    #(
        parameter int ENTRIES = 16          // 0 removes the check entirely
    )
    (
    ...
    function automatic logic match_one(input logic [53:0] a,
        ...
        logic [53:0] napot_mask;
        begin
            // the bits from the lowest zero of pmpaddr downwards are the ones
            // inside the region; all ones matches the whole address space
    ...
    always @(*) begin
        hit_lo = 1'b0;  idx_lo = 0;
        hit_hi = 1'b0;  idx_hi = 0;
        // counted downwards so that the lowest numbered match is the one
        // that is left standing
        for (int i = ENTRIES-1; i >= 0; i--) begin
            logic [53:0] this_a, prev_a;
            logic [1:0]  mode;
            this_a = addr[64*i +: 54];
            prev_a = (i == 0) ? 54'd0 : addr[64*(i-1) +: 54];
            mode   = cfg[8*i+3 +: 2];
            if (match_one(a_lo, this_a, prev_a, mode)) begin hit_lo = 1'b1; idx_lo = i; end
            if (match_one(a_hi, this_a, prev_a, mode)) begin hit_hi = 1'b1; idx_hi = i; end
        end
    end

    always @(*) begin
        win_cfg = cfg[8*idx_lo +: 8];
        ...
        else if (hit_lo != hit_hi)         fail = 1'b1;   // half in, half out
        else if (idx_lo != idx_hi)         fail = 1'b1;   // across two regions
        else                               fail = ~perm_ok;
    end
